...
### Favicon spinner tracks both in-flight API requests and route-segment loading

`frontend/src/lib/loadingIndicator.ts` is a tiny module-level pub/sub (not React context — it's driven from outside any single component tree) combining two independent signals into one loading boolean:
...
- A **route-transition flag**, started/stopped by `RouteLoading` (`frontend/src/components/RouteLoading.tsx`) mounting/unmounting. It's a boolean, not a counter, in case a segment's fallback ever mounts more than once during one transition. `RouteLoading` is meant to be dropped into a route segment's `loading.tsx` (currently `admin/loading.tsx`) — Next.js wraps that segment in a Suspense boundary and renders it as the fallback the instant navigation begins, unmounting it the moment the real page takes over. That's a framework-guaranteed signal, unlike the first version of this feature, which tried to detect navigation by patching `history.pushState`/`replaceState` — that never actually fired, almost certainly because the App Router's internal navigation code captured its own reference to the original `pushState` before our patch ran, so the patched version was simply never called. `loading.tsx` sidesteps the problem entirely by using Next's own Suspense-based mechanism instead of guessing at when navigation starts.

The API-only version of this (before `RouteLoading` existed) missed exactly the case it was built for: in dev mode, navigating to a route not yet compiled can take several seconds *before* the new page's own `useEffect` ever fires a fetch — that whole window had no feedback. `FaviconLoadingIndicator` (mounted once in the root layout) subscribes to the combined signal and swaps the tab's `<link rel="icon">` href to a small rotating SVG (data URI, no network round-trip) while either signal is active, restoring the original href once both clear. There's also a 200ms delay before the spinner appears, so a fast request or transition doesn't cause a visible flash-then-restore flicker.
